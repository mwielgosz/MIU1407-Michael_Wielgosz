...
NSString * Wielgosz_Michael_UtilityApplication$ModuleRequireFormat = @"(function(exports){"
...
@implementation Wielgosz_Michael_UtilityApplicationObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_wielgosz_michael_utilityapplication);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_wielgosz_michael_utilityapplication gc];
}
...
{
	// create Wielgosz_Michael_UtilityApplication global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Wielgosz_Michael_UtilityApplication" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_wielgosz_michael_utilityapplication = [[Wielgosz_Michael_UtilityApplicationObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_wielgosz_michael_utilityapplication];
	
	NSString *_wielgosz_michael_utilityapplicationNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _wielgosz_michael_utilityapplicationNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_wielgosz_michael_utilityapplication valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Wielgosz_Michael_UtilityApplication$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.wielgosz_michael_utilityapplication.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.wielgosz_michael_utilityapplication.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
