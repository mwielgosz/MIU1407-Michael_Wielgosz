...
//Rare exceptions to indicate a bug in the _wielgosz_michael_utilityapplication code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_wielgosz_michael_utilityapplication = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_wielgosz_michael_utilityapplication]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.wielgosz_michael_utilityapplication.module"
