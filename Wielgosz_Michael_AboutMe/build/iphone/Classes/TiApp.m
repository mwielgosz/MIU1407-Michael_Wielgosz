...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kWielgosz_Michael_AboutMeUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kWielgosz_Michael_AboutMeUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Wielgosz_Michael_AboutMe app using the node.js CLI
