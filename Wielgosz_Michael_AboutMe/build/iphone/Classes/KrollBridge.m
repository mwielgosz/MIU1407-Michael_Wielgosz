...
NSString * Wielgosz_Michael_AboutMe$ModuleRequireFormat = @"(function(exports){"
...
@implementation Wielgosz_Michael_AboutMeObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_wielgosz_michael_aboutme);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Wielgosz_Michael_AboutMe global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Wielgosz_Michael_AboutMe" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_wielgosz_michael_aboutme = [[Wielgosz_Michael_AboutMeObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_wielgosz_michael_aboutme];
	
	NSString *_wielgosz_michael_aboutmeNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _wielgosz_michael_aboutmeNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_wielgosz_michael_aboutme valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Wielgosz_Michael_AboutMe$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.wielgosz_michael_aboutme.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.wielgosz_michael_aboutme.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
