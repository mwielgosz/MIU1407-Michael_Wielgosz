...
NSString * Wielgosz_Michael_InformationalApp$ModuleRequireFormat = @"(function(exports){"
...
@implementation Wielgosz_Michael_InformationalAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_wielgosz_michael_informationalapp);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_wielgosz_michael_informationalapp gc];
}
...
{
	// create Wielgosz_Michael_InformationalApp global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Wielgosz_Michael_InformationalApp" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_wielgosz_michael_informationalapp = [[Wielgosz_Michael_InformationalAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_wielgosz_michael_informationalapp];
	
	NSString *_wielgosz_michael_informationalappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _wielgosz_michael_informationalappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_wielgosz_michael_informationalapp valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Wielgosz_Michael_InformationalApp$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.wielgosz_michael_informationalapp.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.wielgosz_michael_informationalapp.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
